		      ...
		      const wchar_t *type = L"InprocServer32")
{
	...
	if (StringFromGUID2(cls, cls_str, CHARS_IN_GUID) == 0)
	{
		goto fail;
	}
	...
	if (RegCreateKeyW(HKEY_CLASSES_ROOT, temp, &key) != ERROR_SUCCESS) {
		goto fail;
	}

	RegSetValueW(key, nullptr, REG_SZ, desc, string_size(desc));

	if (RegCreateKeyW(key, type, &subkey) != ERROR_SUCCESS) {
		goto fail;
	}
...
static bool RegServers(bool reg)
{
	...
	if (reg) 
	{
		return RegServer(CLSID_OBS_VirtualVideo, L"Virtual Camera",
				 file);
	} 
	else 
	{
